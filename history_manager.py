...
def compare_data(old_novel_data, new_novel_data):
    ...
    if new_general_lastup > old_general_lastup:
        notification_pusher.new_chapter_notification(new_novel_data["writer"], new_novel_data["title"], new_novel_data["ncode"])
        updated = True
    elif new_novelupdated_at > old_novelupdated_at:
        config = update_scheduler.read_config()
        if config["novel_modified_notifications"]:
            notification_pusher.novel_modified_notification(new_novel_data["writer"], new_novel_data["title"], new_novel_data["ncode"])
            updated = True
    return updated
